...
<body>
  ...
  class Page
  {
    ...
    {
      $header = '<h1 class="header">' . $this->title . '</h1><hr><hr>';
      $this->page .= $header;
    }
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="get">
      <div>
        <div class="input__container">
          ...
          <?php print '<input type="text" name="title" required value="' . $title . '"/>'
          ?>
        </div>
        <div class="input__container">
          <label label>Copyright information</label>
          <?php print '<input type="text" name="copyright" required value="' . $copyright . '"/>'
          ?>
        </div>
        <div class="input__container">
          <label label>Publicized year</label>
          <?php print '<input type="text" name="year" required value="' . $year . '"/>'
          ?>
        </div>
        <div class="input__container">
